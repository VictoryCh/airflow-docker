...
class Postgres2SMB_cos(BaseOperator):
    ...
    def __init__(self, sql, postgres_conn_id=None, parameters=None, autocommit=False, rows_chunk=5000,
                 ...
                 encoding='utf-8', modewr='wb', SheetName='Лист1', date_format='dd.mm.yyyy', template_excel_xml=None, decrypt_col=None, *args, **kwargs):
    ...
    def execute(self, context):
        target_conn = BaseHook.get_connection(self.samba_conn_id)
        conn = SMBConnection(target_conn.login, target_conn.password, "airflow_worker", target_conn.host, use_ntlm_v2=True)
        assert conn.connect(target_conn.host, 445)
        ...
        try:
            ...
            self._execute(src_hook, conn, context)
    ...
    def give_xls_output(self, src_conn, dest_hook, dir_file, context):
        ...
        with dest_hook.open(dir_file, mode=self.modewr) as tfile:
            wb.save(tfile)
        cursor.close()
        return q_rows
