    ...
    while i <= 2:
        ...
        if i == 1:
            transfer_cos_smb = Postgres2SMB_cos(
                ...
                sql=f'''select WERKS, BTRTL, MOFID, BTEXT, NAME1 from {SCHEMA_source}.biq_6611_region_f('{EXEC_DATE}');''',
        ...
        if i == 2:
            transfer_cos_db = Postgres2Postgres(
                ...
                dest_table='sap_hr_region',
                dest_cols=['WERKS', 'BTRTL', 'MOFID', 'BTEXT', 'NAME1'],
                on_success_callback=task_success_log,
                trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_region;'''
            )
