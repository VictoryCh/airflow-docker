...
from operators.Postgres2Postgres import Postgres2Postgres
    ...
    while i <= 2:
        ...
        if i == 1:
            transfer_cos_smb = Postgres2SMB_cos(
                ...
                sql=f'''select to_char(now(), 'yyyymmdd') as "date", to_char(now(), 'hh24:mi:ss') as "time", WERKS, BTRTL, ORGEH, LNAME, SNAME, ABBRV, OPRNT, CHPER, ZTYPE, SENDD
                        from {SCHEMA_source}.biq_6611_orgunit_new_f('{EXEC_DATE}')''',
                headings=['DATE', 'TIME', 'WERKS', 'BTRTL', 'ORGEH', 'LNAME', 'SNAME', 'ABBRV', 'OPRNT', 'CHPER', 'ZTYPE', 'SENDD'],
                samba_conn_id=CONN_target_smb,
                share=SMB_share,
                dir_samba=DIR_SMB,
                file_name='orgunit_new',
                file_format='xml',
                template_excel_xml='./template/xml/biq_6611/orgunit_new.xml',
                encoding='utf-8',
        ...
        if i == 2:
            transfer_cos_db = Postgres2Postgres(
                ...
                dest_table='sap_hr_orgunit_new',
                dest_cols=['DATE', 'TIME', 'WERKS', 'BTRTL', 'ORGEH', 'LNAME', 'SNAME', 'ABBRV', 'OPRNT', 'CHPER',
                          'ZTYPE', 'SENDD'],
                on_success_callback=task_success_log,
                trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_orgunit_new;'''
            )
