    ...
    while i <= 2:
        ...
        if i == 1:
            transfer_cos_smb = Postgres2SMB_cos(
                ...
                sql=f'''select WERKS, BTRTL, UTCDIFF, UTCSIGN, DESCRIPT from {SCHEMA_source}.TIMEZONE;''',
        ...
        if i == 2:
            transfer_cos_db = Postgres2Postgres(
                ...
                dest_table='sap_hr_timezone',
                dest_cols=['WERKS', 'BTRTL', 'UTCDIFF', 'UTCSIGN', 'DESCRIPT'],
                on_success_callback=task_success_log,
                trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_timezone;'''
            )
