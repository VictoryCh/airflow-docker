...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select pernr, begda, endda, sprps, abtype, abname 
                from {SCHEMA_source}.absence_actual_f('{EXEC_DATE}')''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_absence_actual',
        dest_cols=['pernr', 'begda', 'endda', 'sprps', 'abtype', 'abname'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_absence_actual;'''
    )
