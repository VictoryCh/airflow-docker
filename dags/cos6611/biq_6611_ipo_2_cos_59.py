...
if deployment == "DEV":
    ...
    ST = datetime(2022, 6, 16)  # Заменяем на дату внедрения интеграции
    SI = '0 0 * * *'  # Заменяем на необходимый шедулинг
...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select pernr, delflag, persname, persbegda, persendda, persgbdat, orgeh, plans, werks, btrtl, persg, persk 
                from {SCHEMA_source}.personal_data_actual_f('{EXEC_DATE}')''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_personal_data_actual',
        dest_cols=['pernr', 'delflag', 'persname', 'persbegda', 'persendda', 'persgbdat', 'orgeh', 'plans', 'werks', 'btrtl', 'persg', 'persk'],
        decrypt_column=['persname', 'persgbdat'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_personal_data_actual;'''
    )
