...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select objid, delflag, posname, posbegda, posendda, status, werks, btrtl 
                from {SCHEMA_source}.position_actual_f('{EXEC_DATE}')''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_position_actual',
        dest_cols=['objid', 'delflag', 'posname', 'posbegda', 'posendda', 'status', 'werks', 'btrtl'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_position_actual;'''
    )
