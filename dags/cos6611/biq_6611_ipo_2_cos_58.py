...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select orgid, begda, endda, posid 
                from {SCHEMA_source}.rate_unit_position_actual_f('{EXEC_DATE}')''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_rate_unit_position_actual',
        dest_cols=['orgid', 'begda', 'endda', 'posid'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_rate_unit_position_actual;'''
    )
