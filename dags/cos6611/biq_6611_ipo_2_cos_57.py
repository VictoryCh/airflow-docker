...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select objid, ratebegda, rateendda, pernr 
                from {SCHEMA_source}.chief_position_actual_f('{EXEC_DATE}')''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_chief_position_actual',
        dest_cols=['objid', 'ratebegda', 'rateendda', 'pernr'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_chief_position_actual;'''
    )
