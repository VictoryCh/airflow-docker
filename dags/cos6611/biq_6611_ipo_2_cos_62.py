...
if deployment == "DEV":
    ...
    ST = datetime(2021, 5, 15)  # Заменяем на дату внедрения интеграции
    SI = '0 16 16 Jun *'  # Заменяем на необходимый шедулинг
    ...
    CONN_target = 'COS_DB'
    SCHEMA_target = 'public'
    ST = datetime(2021, 12, 19)  # Заменяем на дату внедрения интеграции
    SI = '0 0 20 Dec *'
...
with DAG(
    ...
    transfer_cos = Postgres2Postgres_cos(
        ...
        sql=f'''select mofid, date, holiday, txt_long 
                from {SCHEMA_source}.calendar_on_request_v''',
        postgres_dest_conn_id=CONN_target,
        dest_schema=SCHEMA_target,
        dest_table='sap_hr_calendar_on_request',
        dest_cols=['mofid', 'date', 'holiday', 'txt_long'],
        on_success_callback=task_success_log,
        trunc=f'''TRUNCATE TABLE {SCHEMA_target}.sap_hr_calendar_on_request;'''
    )
